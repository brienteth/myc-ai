...
logger = logging.getLogger("myca_intelligence.automation.mcp")
    ...
    async def connect(self):
        try:
            ...
            extra_paths = [
                ...
                os.path.expanduser("~/.nvm/versions/node/v20.18.0/bin"),
                os.path.expanduser("~/.nvm/versions/node/v20.0.0/bin"),
                os.path.expanduser("~/.nvm/versions/node/v18.20.0/bin"),
            ...
            bin_name = args[0]
            resolved_bin = shutil.which(bin_name, path=env["PATH"])
            if resolved_bin:
                args[0] = resolved_bin

            # If the user passed an installer command (e.g. npx -y ... install --api-key=...),
            # run the installer script first to completion, then load the installed bridge executable!
            if "install" in args and any(arg.startswith("--api-key=") for arg in args):
                logger.info(f"[MCP CLIENT] Running installer command first: {' '.join(args)}")
                ...
                logger.info(f"[MCP CLIENT] Installer finished. Output: {out.decode()[:200]}")

                # Read the generated bridge command from ~/.cursor/mcp.json or default locations
                    ...
                    try:
                        with open(mcp_json_path, "r", encoding="utf-8") as f:
                            ...
                            for s_name, s_cfg in mcp_servers.items():
                                if "opacus" in s_name or "kinetic" in s_name:
                                    cmd_bin = shutil.which(s_cfg["command"], path=env["PATH"]) or s_cfg["command"]
                                    args = [cmd_bin] + s_cfg.get("args", [])
                                    logger.info(f"[MCP CLIENT] Resolved installed stdio server args: {args}")
                                    break
                    except Exception as ex_json:
                        logger.warning(f"[MCP CLIENT] Failed to parse generated mcp.json: {ex_json}")
