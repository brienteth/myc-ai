...
class BackendRegistry:
    ...
    def create_backend(cls, name: str = "auto") -> InferenceEngine:
        ...
        """
        try:
            import myca.inference.backends
        ...
        # 1. Try Ollama if running and has models installed
            ...
            try:
                ...
                with urllib.request.urlopen(req, timeout=1.0) as resp:
                    if resp.status == 200:
                        ...
                        if data.get("models"):
                            logger.info("Auto-detected local Ollama instance with models. Using OllamaBackend.")
                            return cls._backends["ollama"]()
            except Exception:
                pass

        # 2. Try LlamaCpp if installed and local GGUF model exists
        if "llamacpp" in cls._backends:
            try:
                import llama_cpp
                llamacpp_cls = cls._backends["llamacpp"]
                model_path = os.getenv("MYCA_MODEL_PATH", None)
                if model_path and model_path.startswith("sk-"):
                    model_path = None
                gpu_layers = int(os.getenv("MYCA_GPU_LAYERS", "-1"))
                ctx_size = int(os.getenv("MYCA_CTX", "4096"))
                verbose = os.getenv("MYCA_VERBOSE", "").lower() == "true"
                instance = llamacpp_cls(
                    model_path=model_path,
                    n_gpu_layers=gpu_layers,
                    n_ctx=ctx_size,
                    verbose=verbose
                )
                logger.info(f"Auto-detected LlamaCppBackend with local model: {instance.model_path}")
                return instance
            except ImportError:
                pass
            except Exception as e:
        ...
        # 3. Check 0G Compute Network cloud key (MYCA_MODEL_PATH=sk-... or ZG_COMPUTE_API_KEY)
