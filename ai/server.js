...
import express from './core/mini_express.js';
...
import { OllamaBridge } from './core/ollama_bridge.js';

// Global Ollama bridge instance
...
app.post('/licensing/webhook', async (req, res) => {
  try {
    ...
    const secret = process.env.HELIO_WEBHOOK_SECRET || process.env.MYCAI_WEBHOOK_SECRET || '';
    if (!secret) return res.status(500).json({ error: 'Webhook secret not configured in environment' });
...
// POST /v1/chat/completions (OpenAI Compatible + Hybrid Reasoning Router)
app.post('/v1/chat/completions', async (req, res) => {
  try {
    ...
    let usedEngine = 'spectral';
    let ollamaMeta = null;
    
    // Handle Ollama delegation
    if (routeResult.route === 'ollama') {
      try {
        const ollamaAvailable = await ollamaBridge.isAvailable();
        if (ollamaAvailable) {
          // Build memory context if available
          let memoryContext = null;
          if (sdk.router && sdk.router.memory) {
            try {
              const memResult = sdk.router.memory.query(prompt);
              if (memResult && memResult.results && memResult.results.length > 0) {
                memoryContext = memResult.results
                  .slice(0, 3)
                  .map(r => r.value || r.text || '')
                  .filter(Boolean)
                  .join('\n');
              }
            } catch (_) { /* memory query optional */ }
          }

          const ollamaResult = await ollamaBridge.generate(prompt, {
          ...
          ollamaMeta = {
            ...
            tokensPerSec: ollamaResult.tokensPerSec,
            totalDuration: ollamaResult.totalDuration,
            evalCount: ollamaResult.evalCount
          };
        } else {
          // Ollama not available — use spectral attempt or generic fallback
          content = routeResult.spectralAttempt || 
            'Şu anda yerel dil modeli yüklenmiyor. Lütfen tekrar deneyin.';
          usedEngine = 'spectral-fallback';
        }
      } catch (ollamaErr) {
        console.error('[Ollama Error]', ollamaErr.message);
        content = routeResult.spectralAttempt || 
          'Yerel model yanıt üretemedi. Lütfen tekrar deneyin.';
        usedEngine = 'spectral-fallback';
      }
    } else if (routeResult.route === 'llm' && routeResult.sdkResult && routeResult.sdkResult.newTokens) {
      content = routeResult.sdkResult.newTokens.join(" ");
      usedEngine = 'spectral-slm';
    }
    ...
    const responseBody = {
      id: `chatcmpl-hybrid-${Math.random().toString(36).substring(2, 10)}`,
      object: "chat.completion",
      created: Math.floor(Date.now() / 1000),
      model: model || `myca-hybrid-${usedEngine}`,
      choices: [{
        index: 0,
        message: {
          role: "assistant",
...
// POST /fetch-url — Proxy fetch web documents / PDFs
  ...
  try {
    ...
    const request = client.get(targetUrl, {
      ...
      if (resp.statusCode >= 300 && resp.statusCode < 400 && resp.headers.location) {
        const redirectUrl = new URL(resp.headers.location, targetUrl).toString();
        return res.redirect(redirectUrl);
      }

      if (resp.statusCode >= 400) {
        return res.status(resp.statusCode).json({ error: `HTTP ${resp.statusCode} from target URL` });
      }
...
const isMain = process.argv[1] && (
  ...
  process.argv[1].endsWith('ai/server.js')
);

if (isMain) {
  app.listen(PORT, () => {
    console.log(`=== TURKISH RESONANCE AI CORE SERVER ACTIVE ON http://localhost:${PORT} ===`);
  });
}
