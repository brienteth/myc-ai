...
export class ReasoningRouter {
  ...
  _routeNormal(input, options = {}) {
    ...
    // 0. Doğal Selamlaşma ve Karşılama (Conversational Greeting)
    const greetRegex = /^(merhaba|selam|günaydın|iyi günler|iyi akşamlar|selamlar|nasılsın|naber|kimsin|ne yapabilirsin|yardım|hey|hi|hello)\b/i;
    if (greetRegex.test(cleanInput)) {
      return {
        input: cleanInput,
        route: 'dialogue',
        llmBypassed: true,
        output: "Merhaba! Ben Myca'nın yerel Hiperboyutlu Rezonans motoruyla çalışan otonom ajanıyım. Bulut sunuculara veya ücretli API'lere bağlanmadan doğrudan cihazınızda %100 yerel çalışıyorum. Endüstriyel donanım, otomasyon, kod geliştirme veya kurumsal operasyonlarda size nasıl yardımcı olabilirim?",
        reason: 'Conversational greeting intent'
      };
    }
    ...
    // 4. Intent Similarity Check — route low-confidence to Ollama LLM
    ...
    if (maxSimilarity < 0.12) {
      // No spectral intent matched → delegate to local Ollama LLM
      return {
        input: cleanInput,
        route: 'ollama',
        llmBypassed: false,
        output: null, // Server will fill via OllamaBridge
        confidence: maxSimilarity,
        reason: `No spectral intent matched (max: ${maxSimilarity.toFixed(4)}) → delegating to local Ollama LLM`
      };
    }

    // 5. Serbest Dil Üretimi — SpectralSLM önce dener, başarısız olursa Ollama'ya yönlendirir
    let outputText = '';
    let sdkResult = null;
    if (this.sdk && typeof this.sdk.generate === 'function') {
      // Call local SpectralSLM first
    ...
    const candidateGenResult = {
      ...
      output: outputText,
      sdkResult,
      reason: 'Generative language synthesis (SpectralSLM)'
    };

    // If SpectralSLM output passes 8-Axis validation, use it directly
    if (outputText && this.validateAllAxes(candidateGenResult)) {
      return candidateGenResult;
    }

    // SpectralSLM failed validation or produced empty output → delegate to Ollama
    return {
      input: cleanInput,
      route: 'ollama',
      llmBypassed: false,
      output: null, // Server will fill via OllamaBridge
      spectralAttempt: outputText || null,
      sdkResult,
      reason: 'SpectralSLM output failed 8-Axis validation → delegating to Ollama LLM'
    };
  }
