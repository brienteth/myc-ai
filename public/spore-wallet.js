...
let activeWallet = {
  type: 'none', // 'extension' | 'native' | 'metamask' | 'founder' | 'none'
  evmAddress: null,
  mycAddress: null
};

// ═══════════════════════════════════════════════════════════════════
// TOKENOMICS & TESTNET SUPPLY SPECIFICATION
// Total Supply: 1,000,000,000 MYC (1 Billion)
// Founder Allocation (20%): 200,000,000 MYC (myc1af3a246683298d473cb2534a78a5f31a)
// Faucet & AMM Contract Reserve (80%): 800,000,000 MYC (0x...fAUc108)
// Standard Wallets: 0.00 MYC, 0.00 USDT, 0.00 USDC initial balance (Must claim from Faucet)
// ═══════════════════════════════════════════════════════════════════
const FOUNDER_WALLET_ADDRESS = 'myc1af3a246683298d473cb2534a78a5f31a';
const TOTAL_TESTNET_SUPPLY = {
  MYC: 1000000000.0,
  USDT: 10000000.0,
  USDC: 10000000.0
};
const FOUNDER_ALLOCATION = {
  MYC: 200000000.0,
  USDT: 2000000.0,
  USDC: 2000000.0
};
const FAUCET_CONTRACT_RESERVE = {
  MYC: 800000000.0,
  USDT: 8000000.0,
  USDC: 8000000.0
};

function isFounderAddress(addr) {
  if (!addr) return false;
  const clean = addr.toLowerCase().trim();
  return clean === FOUNDER_WALLET_ADDRESS.toLowerCase() || 
         clean.startsWith('myc1founder') || 
         clean.includes('genesis_founder');
}

function getWalletBalances(addr) {
  if (!addr) return { MYC: 0.0, USDT: 0.0, USDC: 0.0 };
  const clean = addr.toLowerCase().trim();
  
  if (isFounderAddress(clean)) {
    try {
      const saved = localStorage.getItem('myca_balances_' + clean);
      if (saved) {
        const parsed = JSON.parse(saved);
        if (parsed && typeof parsed.MYC === 'number') return parsed;
      }
    } catch(e) {}
    return { ...FOUNDER_ALLOCATION };
  }

  // Standard wallet: strictly 0.00 unless claimed from Faucet
  try {
    const saved = localStorage.getItem('myca_balances_' + clean);
    if (saved) {
      const parsed = JSON.parse(saved);
  ...
  return { MYC: 0.0, USDT: 0.0, USDC: 0.0 };
}

let currentBalances = { MYC: 0.0, USDT: 0.0, USDC: 0.0 };

function syncActiveWalletBalances() {
  const addr = getActiveWalletAddress();
  currentBalances = getWalletBalances(addr);
}
...
function saveUserBalances() {
  ...
  try {
    const clean = addr.toLowerCase().trim();
    localStorage.setItem('myca_balances_' + clean, JSON.stringify(currentBalances));
...
function updateBalancesUI() {
  try {
    const mycBal = (currentBalances && currentBalances.MYC !== undefined) ? currentBalances.MYC : 0.0;
    const navBal = document.getElementById('nav-wallet-balance');
    if (navBal) {
      navBal.innerText = parseFloat(mycBal).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + ' MYC';
    }
    const sporeBal = document.getElementById('spore-balance-display');
    if (sporeBal) {
      sporeBal.innerHTML = parseFloat(mycBal).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + ' <span class="currency">$MYC</span>';
    }
    ...
    if (fromTokEl) {
      ...
      const val = (currentBalances && currentBalances[tok] !== undefined) ? currentBalances[tok] : 0.0;
      if (el) el.innerText = val.toLocaleString('en-US', {minimumFractionDigits: 2}) + ' ' + tok;
      const balDisplay = document.getElementById('swap-from-bal-display');
      if (balDisplay) balDisplay.innerText = 'Balance: ' + val.toLocaleString('en-US', {minimumFractionDigits: 2}) + ' ' + tok;
    }
    if (toTokEl) {
      ...
      const val = (currentBalances && currentBalances[tok] !== undefined) ? currentBalances[tok] : 0.0;
      if (el) el.innerText = val.toLocaleString('en-US', {minimumFractionDigits: 2}) + ' ' + tok;
      const balDisplay = document.getElementById('swap-to-bal-display');
      if (balDisplay) balDisplay.innerText = 'Balance: ' + val.toLocaleString('en-US', {minimumFractionDigits: 2}) + ' ' + tok;
    }
    const bridgeBalEl = document.getElementById('bridge-user-balance');
    if (bridgeBalEl) bridgeBalEl.innerText = parseFloat(mycBal).toLocaleString('en-US', {minimumFractionDigits: 2}) + ' MYC';
    const stakeBalEl = document.getElementById('stake-available-balance');
    if (stakeBalEl) stakeBalEl.innerText = parseFloat(mycBal).toLocaleString('en-US', {minimumFractionDigits: 2}) + ' MYC';

    if (typeof checkSwapBalance === 'function') {
      checkSwapBalance();
    }
  } catch(e) {}
}
...
function openWalletKitModal() {
  ...
  const detectedAddr = (window.myc && window.myc.selectedAddress) || window.__MYCA_INITIAL_ADDRESS__ || '';

  if (isExtensionInstalled) {
    if (statusBadge) statusBadge.innerHTML = '<span style="color:#00e87a; font-weight:700;">● Extension Detected in Chrome ⚡</span>';
    if (actionBox) {
      const addrSnippet = detectedAddr
        ? `<div style="background: rgba(0,0,0,0.4); border: 1px solid rgba(0, 240, 255, 0.25); border-radius: 8px; padding: 6px 10px; font-family: monospace; font-size: 11px; color: #00f0ff; display: flex; align-items: center; justify-content: space-between; margin-bottom: 4px;">
             <span>${detectedAddr.slice(0, 12)}...${detectedAddr.slice(-6)}</span>
             <span style="color: #10b981; font-weight: 700; font-size: 10px;">PUF ACTIVE</span>
           </div>`
        : '';

      actionBox.innerHTML = `
        ${addrSnippet}
        <button onclick="connectSporeExtension()" style="width: 100%; background: linear-gradient(135deg, #00f0ff, #10b981); color: #000; font-weight: 800; border: none; padding: 12px; border-radius: 10px; font-size: 13.5px; cursor: pointer; box-shadow: 0 4px 16px rgba(0, 240, 255, 0.4); display: flex; align-items: center; justify-content: center; gap: 8px;">
          <span>⚡</span> Connect &amp; Sign with Spore Wallet
        </button>
        <div style="display:flex; justify-content: flex-end; margin-top:4px;">
          <a href="/spore-wallet-extension.zip" download="spore-wallet-extension.zip" style="font-size: 10.5px; color: #64748b; text-decoration: none;">
    ...
    if (statusBadge) statusBadge.innerHTML = '<span style="color:#f59e0b; font-weight:600;">● Extension Not Detected (Refresh after install)</span>';
    if (actionBox) {
      actionBox.innerHTML = `
        ...
        <div id="ext-install-guide" style="display:none; background: rgba(0,0,0,0.5); border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; padding: 10px; font-size: 11px; color: #94a3b8; line-height: 1.5; margin-top: 4px; text-align: left;">
          ...
          4. <b>Refresh this page (F5)</b> and click Connect &amp; Sign with Spore Wallet!
        </div>
      `;
    }
  }
  ...
  if (typeof window.myc !== 'undefined' && window.myc && typeof window.myc.connect === 'function') {
    try {
      ...
      if (accounts && accounts.length > 0) {
        ...
        updateWalletUI();
        closeWalletModal();
        showToast('Connected & Signed!', 'Authenticated via Spore Extension: ' + addr.slice(0, 10) + '...', 'success');
        return;
      }
    } catch(e) {
...
function updateWalletUI() {
  ...
  if (activeWallet.type === 'founder' || isFounderAddress(activeWallet.mycAddress)) {
    if (icon) icon.innerText = '👑';
    const addr = activeWallet.mycAddress;
    if (label) label.innerText = addr ? `👑 ${addr.slice(0, 6)}...${addr.slice(-4)}` : 'Founder (20%)';
...
function updateSporeWalletPanel() {
  ...
  if (activeWallet.type === 'founder' || isFounderAddress(addr)) {
    if (titleEl) titleEl.innerText = 'FOUNDER GENESIS NODE';
    if (typeEl) typeEl.innerHTML = '<span style="color:#fbbf24; font-weight:700;">👑 Founder Genesis (20% Supply Allocation: 200,000,000 MYC)</span>';
    if (pufBadge) pufBadge.innerText = 'FOUNDER 20%';
  ...
  var bal = (currentBalances && currentBalances.MYC !== undefined) ? currentBalances.MYC : 0.0;
...
async function executeSporeTransfer() {
  ...
  if (amount > (currentBalances.MYC || 0)) { showToast('Insufficient', 'Only have ' + (currentBalances.MYC || 0) + ' MYC. Claim from Faucet!', 'error'); return; }
...
function connectFounderWallet() {
  activeWallet = {
    type: 'founder',
    evmAddress: toEvmAddress(FOUNDER_WALLET_ADDRESS),
    mycAddress: FOUNDER_WALLET_ADDRESS
  };
  try {
    localStorage.setItem('myca_wallet_type', 'founder');
    localStorage.setItem('myca_native_address', FOUNDER_WALLET_ADDRESS);
  } catch(e) {}
  syncActiveWalletBalances();
  updateWalletUI();
  updateSporeWalletPanel();
  updateBalancesUI();
  closeWalletModal();
  showToast('Founder Genesis Active 👑', 'Connected to Founder Node (200,000,000 MYC - 20% Allocation)', 'success');
}
...
async function executeActivateCreatedWallet() {
  ...
  updateSporeWalletPanel();
  updateBalancesUI();
  showToast('Wallet Created (0.00 MYC)', 'Standard wallet initialized. Please claim testnet tokens from Faucet!', 'info');
}
...
async function executeImportSeed() {
  ...
  if (words.length !== 12) {
    showToast('Invalid Seed', 'Seed must contain exactly 12 words', 'error');
    return;
  }
...
function initSporeWallet() {
  ...
  // Wipe legacy fake 'native' sandbox wallet so user is never stuck on a random address
  if (savedType === 'native') {
    try {
      localStorage.removeItem('myca_wallet_type');
      localStorage.removeItem('myca_native_address');
  ...
  // Auto-connect ONLY if user previously authenticated Spore extension or MetaMask
  if (savedType === 'founder') {
    activeWallet = {
      type: 'founder',
      evmAddress: toEvmAddress(FOUNDER_WALLET_ADDRESS),
      mycAddress: FOUNDER_WALLET_ADDRESS
    };
  } else if (savedType === 'extension') {
    const extAddr = (window.myc && window.myc.selectedAddress) || window.__MYCA_INITIAL_ADDRESS__ || savedMyc;
    if (extAddr) {
      activeWallet = {
        type: 'extension',
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const syncExtensionAddress = (addr) => {
    const freshAddr = addr || (window.myc && window.myc.selectedAddress) || window.__MYCA_INITIAL_ADDRESS__;
    if (freshAddr) {
      const savedType = localStorage.getItem('myca_wallet_type');
      if (savedType === 'extension' || activeWallet.type === 'extension') {
        activeWallet.type = 'extension';
        activeWallet.mycAddress = freshAddr;
        activeWallet.evmAddress = toEvmAddress(freshAddr);
        try { localStorage.setItem('myca_native_address', freshAddr); } catch(e) {}
        syncActiveWalletBalances();
        updateWalletUI();
        updateSporeWalletPanel();
        updateBalancesUI();
  ...
  window.addEventListener('myc#initialized', (e) => {
    syncExtensionAddress(e.detail && e.detail.address);
    if (typeof window.myc !== 'undefined' && window.myc && typeof window.myc.on === 'function') {
      window.myc.on('accountsChanged', (accounts) => {
        if (accounts && accounts.length > 0) {
          syncExtensionAddress(accounts[0]);
        }
      });
    }
  });

  window.addEventListener('myc#detected', (e) => {
    syncExtensionAddress(e.detail && e.detail.address);
  });

  window.addEventListener('myc#address_updated', (e) => {
    syncExtensionAddress(e.detail && e.detail.address);
  });
});
