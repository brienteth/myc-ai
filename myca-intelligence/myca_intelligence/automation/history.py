...
class AutomationDB:
    ...
    def init_db():
        ...
        # Pre-seed default production MCP servers
        default_servers = [
            ("chrome-devtools", "Chrome DevTools MCP", "stdio", "npx -y @chrome-devtools/mcp-server", "Disconnected", 0),
            ("github", "GitHub MCP Server", "stdio", "npx -y @modelcontextprotocol/server-github", "Disconnected", 0),
            ("postgres", "PostgreSQL DB Explorer", "stdio", "npx -y @modelcontextprotocol/server-postgres", "Disconnected", 0),
            ("brave", "Brave Web Search MCP", "stdio", "npx -y @modelcontextprotocol/server-brave-search", "Disconnected", 0),
            ("firebase", "Firebase Data Connect MCP", "stdio", "npx -y @firebase/data-connect-mcp", "Disconnected", 0)
        ]
        for s_id, name, s_type, cmd, status, count in default_servers:
            conn.execute("""
                INSERT OR IGNORE INTO mcp_servers (id, name, type, command, status, tools_count, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (s_id, name, s_type, cmd, status, count, time.time()))
