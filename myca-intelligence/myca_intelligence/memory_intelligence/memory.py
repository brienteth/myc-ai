...
logger = logging.getLogger("myca_intelligence.memory_intelligence.memory")
...
class ExperienceMemory:
    ...
    def rank_candidate_dags(self, need_text: str, candidate_dags: list[dict]) -> Tuple[dict, float]:
        """
        Ranks candidate DAGs based on historical execution experience metrics (success rate, latency, cost).
        Does NOT override the Planner, but scores and ranks candidate DAGs.
        ...
        best_score = 0.5  # Base confidence score

        try:
            conn = sqlite3.connect(str(self.db_path))
            rows = conn.execute("SELECT plan_json, success, latency_ms FROM execution_history WHERE success = 1").fetchall()
            conn.close()

            if not rows:
                return best_plan, best_score

            # Score each candidate DAG by matching skills against successful past execution graphs
                ...
                for r in rows:
                    try:
                        ...
                        if skills_in_dag == past_skills:
                            score += 0.4  # High past success match bonus
                    except Exception:
                        pass
                if score > best_score:
                    best_score = score
